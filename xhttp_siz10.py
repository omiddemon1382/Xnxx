# xhttp_siz10.py
# ══════════════════════════════════════════════════════════════════════════════
# MOHAMAD v13.0 · XHTTP Ultra Transport — دو مد: packet-up / stream-up
#  ─ جدید: خروجی TCP از پروکسی (SOCKS5 / HTTP CONNECT) — قابلیت Multi-IP
#    پروکسیِ هر سشن یک‌بار در لحظه‌ی ساخت سشن resolve می‌شود:
#      پروکسی اختصاصی کانفیگ → پروکسی پیش‌فرض سرور → مستقیم
#    و همه‌ی اتصالات TCP آن سشن از داخل تونل همان پروکسی باز می‌شوند؛
#    بنابراین IP خروجی‌ای که سایت‌ها می‌بینند دقیقاً IP همان پروکسی است.
#  ─ موتور تطبیقی stream-up (_AdaptiveFlow AIMD + _QuotaGate تطبیقی) دست‌نخورده
#  ─ ثبت پروکسی استفاده‌شده در connections (نمایش در اتصالات زنده پنل)
# ══════════════════════════════════════════════════════════════════════════════
...
from main import (
    ...
    is_ip_allowed,
    save_state,
    resolve_proxy_for_link,
)
from relay_vless import parse_vless_header, check_and_use
from proxy_outbound import open_connection_via, describe_proxy
...
REAPER_INTERVAL = 10
TCP_CONNECT_TIMEOUT = 15.0   # کمی بیشتر از قبل؛ به‌خاطر هندشیک پروکسی خروجی
...
PACKET_UP_HIGH_WATER = 2 * 1024 * 1024  # packet-up همان منطق ساده‌ی قبلی
...
class _QuotaGate:
    """
    نسخه‌ی تطبیقی: به‌جای await check_and_use() به‌ازای هر چانک، نرخ واقعی ترافیک
    هر سشن رو با EWMA اندازه می‌گیره و اندازه‌ی batch رو زنده عوض می‌کنه:
...
class _AdaptiveFlow:
    ...
    high-water تطبیقی برای drain(), رفتار شبیه AIMD در TCP congestion control:
      ...
      - اگه سریع تموم بشه → سقف بافر مجاز additive increase می‌شه؛ دفعه‌ی بعد
        دیرتر drain صدا زده می‌شه، پس syscall/context-switch کمتر و throughput بالاتر.
      - اگه drain کند بشه (backpressure واقعی) → سقف فوری نصف می‌شه (multiplicative decrease).
    هر سشن یک نمونه‌ی جدا داره؛ مسیرهای کند و سریع تداخلی با هم ندارن.
    """
...
async def _open_tcp_from_header(first_chunk: bytes, proxy: dict | None = None):
    """تونل TCP به مقصدِ هدر VLESS را باز می‌کند — از داخل تونل پروکسی اگر پروکسی باشد."""
    command, address, port, payload = await parse_vless_header(first_chunk)
    reader, writer = await asyncio.wait_for(
        open_connection_via(proxy, address, port), timeout=TCP_CONNECT_TIMEOUT
    )
    _tune_socket(writer)
    if payload:
...
async def _get_or_create_session(uuid: str, mode: str, session_id: str, ip: str = "نامشخص") -> dict:
    ...
    پروکسی خروجی مؤثر، یک‌بار در لحظه‌ی ساخت سشن resolve و در سشن ذخیره می‌شود."""
    async with XHTTP_LOCK:
        ...
        # ── پروکسی خروجی مؤثر این کانفیگ (اختصاصی کانفیگ → پیش‌فرض سرور → مستقیم) ──
        proxy = await resolve_proxy_for_link(uuid)
        proxy_desc = describe_proxy(proxy)
        ...
        sess = {
            ...
            "seq_buf": {}, "next_seq": 0,
            "gate": None,        # لازی ساخته می‌شود: _QuotaGate تطبیقی مخصوص stream-up
            "flow": None,        # لازی ساخته می‌شود: _AdaptiveFlow مخصوص stream-up
            "proxy": proxy,      # پروکسی خروجی این سشن (None = مستقیم)
        }
        xhttp_sessions[session_id] = sess
        logger.info(f"new XHTTP[{mode}] session [{session_id[:8]}] uuid={uuid[:8]} ip={ip} egress={proxy_desc}")
        return sess
...
async def _pump_tcp_to_queue(session_id: str, uuid: str, reader: asyncio.StreamReader, down_q: asyncio.Queue):
    first = True
    gate = _QuotaGate(uuid)  # دانلینک هم از همان گیت batched استفاده می‌کند
    try:
        while True:
...
async def _open_tcp_for_session(session_id: str, uuid: str, sess: dict, first_chunk: bytes):
    """تونل TCP را از روی هدر VLESS باز می‌کند (از داخل پروکسیِ سشن) و پمپ دانلینک را راه می‌اندازد."""
    reader, writer, address, port = await _open_tcp_from_header(first_chunk, sess.get("proxy"))
    logger.info(f"connect XHTTP[{sess['mode']}] [{session_id[:8]}] -> {address}:{port} (via {describe_proxy(sess.get('proxy'))})")
    sess["writer"] = writer
    sess["tcp_open"] = True
...
async def packet_up_upload(uuid: str, session_id: str, seq: int, request: Request):
    ...
    try:
        if sess["writer"] is None:
            # اولین پکتی که حاوی هدر VLESS است، می‌تواند seq=0 نباشد اگر پکت‌ها
            # خارج از ترتیب برسند؛ بافر کوچک برای سورت کردن seqهای زودرس.
            if seq != 0:
                sess["seq_buf"][seq] = body
                return {"ok": True, "buffered": True}
            await _open_tcp_for_session(session_id, uuid, sess, body)
            # هر پکت بافرشده‌ای که حالا نوبتش رسیده را هم بفرست
            nxt = 1
...
# high-water درین) + کش رفرنس‌ها داخل لوپ. هیچ داده‌ای بافر/coalesce نمی‌شود —
# هر بایت فوری write() می‌شود، فقط «کِی صبر کنیم برای drain» تطبیقیه.
...
async def stream_up_upload(uuid: str, session_id: str, request: Request):
    ...
    writer = sess["writer"]                   # ممکن است هنوز None باشد
