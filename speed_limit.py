# speed_limit.py
# ══════════════════════════════════════════════════════════════════════════════
# MOHAMAD v13.0 · محدودیت سرعت (Bandwidth Throttling) به‌ازای هر کانفیگ
#  پیاده‌سازی با الگوی Token Bucket — بدون تغییر منطقی نسبت به نسخه‌ی قبل؛
#  هر دو ماژول relay_vless.py و xhttp_siz10.py این ماژول را صدا می‌زنند.
# ══════════════════════════════════════════════════════════════════════════════
...
# هر uuid یک Bucket جدا دارد؛ Bucket با نرخ صفر (بدون محدودیت) اصلاً ساخته نمی‌شود.
_buckets: dict = {}
...
def reset_bucket(uuid: str):
    """وقتی محدودیت سرعت یک کانفیگ از پنل تغییر کرد یا کانفیگ حذف شد صدا زده می‌شود،
    تا بافر توکن قدیمی پاک شود (نرخ جدید در فراخوانی بعدی throttle از نو ساخته می‌شود)."""
