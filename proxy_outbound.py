...
#  ─ SOCKS5 (RFC 1928 + RFC 1929) با asyncio خالص — بدون وابستگی جدید
#  ─ HTTP CONNECT با fallback: HTTP/1.1 → HTTP/1.0 → پروکسی-TLS (پروکسی https)
...
#  ─ رفع باگ نسخه قبل: بایت‌های بعد از هدر CONNECT دیگر گم نمی‌شوند (readline)
...
async def _socks5_handshake(reader: asyncio.StreamReader,
    ...
    # ۳) BND.ADDR + BND.PORT — بعد از این تونل شفاف است
    atyp_r = head[3]
    try:
        if atyp_r == 0x01:
...
async def _http_connect_handshake(reader: asyncio.StreamReader,
    ...
    # status line (خط‌های خالی ابتدایی نادیده گرفته می‌شوند)
    status_line = b""
    for _ in range(8):
    ...
    # هدرها تا خط خالی؛ بایت‌های اضافه در بافر reader می‌مانند و تونل سالم می‌ماند
    for _ in range(256):
...
# ══════════════════════════════════════════════════════════════════════════════
# نقطه ورود اصلی برای رله‌ها (همان امضای قبلی — drop-in)
# ══════════════════════════════════════════════════════════════════════════════
...
async def open_connection_via(proxy: dict | None,
    ...
    for scheme in _scheme_order(proxy):
        try:
            ...
            _detected[key] = scheme   # پروتکل برنده کش شد
            return reader, writer
        except ProxyError as e:
            last_err = e
            if e.code in (401, 407):
...
async def diagnose_proxy(proxy: dict, timeout: float = 8.0) -> dict:
    """تشخیص مرحله‌به‌مرحله برای دکمه تست: TCP → هندشیک پروتکل → IP خروجی واقعی."""
    stages: list = []
    try:
        phost, pport = _validate(proxy)
    except ProxyError as e:
