...
function jpatch(o){return{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(o)}}
...
function renderLinks(){
  ...
  el.innerHTML=list.map(l=>{
    ...
    COPY_MAP['v-'+l.uuid]=l.vless_link;
    COPY_MAP['s-'+l.uuid]=l.sub_url;
    return `
    <div class="lcard ${ok?'':'off'}">
      <div class="lc-head">
        ...
        <div class="lc-actions">
          ...
          <button class="btn-icon" title="کپی لینک اتصال" onclick="copyKey('v-${l.uuid}')"><i class="ti ti-copy"></i></button>
          <button class="btn-icon" title="کپی لینک ساب" onclick="copyKey('s-${l.uuid}')"><i class="ti ti-rss"></i></button>
      ...
      <div class="lc-badges">
        ...
        <span class="chip chip-gold"><i class="ti ti-route"></i>${PROTO_LABELS[l.protocol]||esc(l.protocol)}</span>
...
async function toggleLink(uid,val){try{await api('/api/links/'+uid,jpatch({active:val}));toast(val?'فعال شد':'غیرفعال شد');loadLinks()}catch(e){toast(e.message,true)}}
async function resetUsage(uid){try{await api('/api/links/'+uid,jpatch({reset_usage:true}));toast('مصرف ریست شد');loadLinks()}catch(e){toast(e.message,true)}}
async function delLink(uid){
  const l=LINKS_CACHE.find(x=>x.uuid===uid);if(!l)return;
  if(!confirm('حذف کانفیگ «'+l.label+'»؟ برگشت‌ناپذیر است.'))return;
  try{await api('/api/links/'+uid,{method:'DELETE'});toast('حذف شد');loadLinks()}catch(e){toast(e.message,true)}
}
 $('btnSubAll').onclick=()=>{navigator.clipboard.writeText(location.origin+'/sub-all').then(()=>toast('لینک ساب همه کپی شد (فقط با لاگین ادمین کار می‌کند)'))};
function showQR(uid){
  const l=LINKS_CACHE.find(x=>x.uuid===uid);if(!l)return;
...
async function loadSubs(){
  try{
    ...
    el.innerHTML=SUBS_CACHE.map(s=>{
      COPY_MAP['gsub-'+s.sub_id]=s.sub_url;
      COPY_MAP['gpub-'+s.sub_id]=s.public_url;
      return `
      <div class="lcard">
        <div class="lc-head">
          ...
          <div class="lc-actions">
            <button class="btn-icon" title="کپی لینک ساب" onclick="copyKey('gsub-${s.sub_id}')"><i class="ti ti-rss"></i></button>
            <button class="btn-icon" title="کپی لینک صفحه" onclick="copyKey('gpub-${s.sub_id}')"><i class="ti ti-copy"></i></button>
        ...
        <div class="lc-badges">
          ...
          <span class="chip"><i class="ti ti-database"></i>${esc(s.total_used_fmt)}</span>
        </div>
        ${s.desc?`<div class="lc-note">${esc(s.desc)}</div>`:''}
        <div class="urlrow"><div class="urltext">${esc(s.sub_url)}</div><button class="btn-icon" onclick="copyKey('gsub-${s.sub_id}')"><i class="ti ti-copy"></i></button></div>
      </div>`;
    }).join('');
  }catch(e){}
}
...
async function saveSub(){
  ...
  try{
    ...
    if(eid){await api('/api/subs/'+eid,jpatch(body));toast('گروه ویرایش شد')}
...
async function delSub(sid){
  const s=SUBS_CACHE.find(x=>x.sub_id===sid);if(!s)return;
  if(!confirm('حذف گروه «'+s.name+'»؟ کانفیگ‌ها حذف نمی‌شوند.'))return;
  try{await api('/api/subs/'+sid,{method:'DELETE'});toast('گروه حذف شد');loadSubs()}catch(e){toast(e.message,true)}
}
function openManage(sid){
 ...
 $('btnSaveManage').onclick=async()=>{
  ...
  try{await api('/api/subs/'+MANAGE_SID,jpatch({link_ids:ids}));toast('کانفیگ‌های گروه ذخیره شد');closeM('mManage');loadSubs()}catch(e){toast(e.message,true)}
};
...
async function loadProxies(){
  try{
    ...
    const dp=$('defProxy');
    const cur=dp.value;
    dp.innerHTML='<option value="">بدون پروکسی (مستقیم)</option>'+PROXIES_CACHE.map(p=>`<option value="${p.proxy_id}">${esc(p.label)} (${esc(p.type)})</option>`).join('');
    dp.value=cur||'';
  }catch(e){}
}
function renderProxies(){
  ...
  el.innerHTML=PROXIES_CACHE.map(p=>{
    COPY_MAP['p-'+p.proxy_id]=p.type+'://'+p.host+':'+p.port;
    return `
    <div class="lcard ${p.active?'':'off'}">
      ...
      <div class="lc-badges">
        <span class="chip chip-gold"><i class="ti ti-route"></i>${esc(p.type.toUpperCase())}</span>
      ...
      <div class="urlrow"><div class="urltext">${esc(p.type)}://${esc(p.host)}:${p.port}</div><button class="btn-icon" onclick="copyKey('p-${p.proxy_id}')"><i class="ti ti-copy"></i></button></div>
...
function editProxy(pid){
 ...
 $('btnSaveProxy').onclick=async()=>{
  ...
  try{
    ...
    if(eid){await api('/api/proxies/'+eid,jpatch(body));toast('پروکسی ویرایش شد')}
...
async function toggleProxy(pid,val){try{await api('/api/proxies/'+pid,jpatch({active:val}));loadProxies()}catch(e){toast(e.message,true)}}
async function delProxy(pid){
  const p=PROXIES_CACHE.find(x=>x.proxy_id===pid);if(!p)return;
  if(!confirm('حذف پروکسی «'+p.label+'»؟ کانفیگ‌های متصل به حالت پیش‌فرض برمی‌گردند.'))return;
  try{await api('/api/proxies/'+pid,{method:'DELETE'});toast('پروکسی حذف شد');loadProxies()}catch(e){toast(e.message,true)}
}
async function makeDefault(pid){
  const p=PROXIES_CACHE.find(x=>x.proxy_id===pid);if(!p)return;
  const next=p.is_default?null:pid;
  try{await api('/api/settings',jpatch({default_proxy_id:next}));toast(next?'پیش‌فرض سرور شد':'از حالت پیش‌فرض خارج شد');loadProxies();loadSettings()}catch(e){toast(e.message,true)}
}
...
async function loadLogs(){
  try{
    ...
    el.innerHTML=logs.map(l=>`
      <div class="lgrow ${cls[l.level]||''}"><i class="ti ${icons[l.kind]||'ti-info-circle'}"></i>
...
async function loadSettings(){
 ...
 $('btnDefProxy').onclick=async()=>{
  try{await api('/api/settings',jpatch({default_proxy_id:$('defProxy').value||null}));toast('پروکسی پیش‌فرض ذخیره شد');loadProxies()}catch(e){toast(e.message,true)}
};
 $('btnPw').onclick=async()=>{
...
/* ═══════════ HUB (فقط پنل مادر نشان داده می‌شود) ═══════════ */
(async function(){
  try{
...
.lc-h{display:flex;align-items:center;gap:9px;margin-bottom:8px;flex-wrap:wrap}
...
<div class="wrap">
  ...
  <div id="content" style="display:none">
    ...
    <div class="card">
      ...
      <div class="subrow"><code id="subUrl">...</code><button class="btn" onclick="copyKey('sub')"><i class="ti ti-copy"></i></button></div>
    </div>
    <div class="card">
...
function copyKey(k){const t=COPY_MAP[k];if(!t)return;navigator.clipboard.writeText(t).then(()=>toast('کپی شد'))}
async function loadData(){
  ...
  try{
    ...
    el.innerHTML=d.links.map(l=>{
      COPY_MAP['l-'+l.uuid]=l.vless_link;
      return `
      <div class="lc">
        <div class="lc-h">
          <div class="lc-n">${esc(l.label)}</div>
          <span class="chip ${l.active?'ok':'bad'}">${l.active?'فعال':'غیرفعال'}</span>
          <button class="btn" style="margin-right:auto;padding:6px 11px;font-size:11px" onclick="copyKey('l-${l.uuid}')"><i class="ti ti-copy"></i></button>
        </div>
        <div class="lc-u">${esc(l.used_fmt)} / ${esc(l.limit_fmt)}${l.proxy_label?' · خروجی: '+esc(l.proxy_label):''}</div>
      </div>`;
    }).join('');
  }catch(e){toast('خطا در بارگذاری')}
}
 $('pwIn').addEventListener('keydown',e=>{if(e.key==='Enter')loadData()});
loadData();
